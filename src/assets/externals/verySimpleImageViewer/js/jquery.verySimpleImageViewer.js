...
(function ($) {
    $.fn.verySimpleImageViewer = function (options) {
        ...
        self.setMouseCursor = function () {
            ...
            if (dimension[0] > frameDimension[0] && dimension[1] > frameDimension[1]) cursor = 'move';
            else if (dimension[0] > frameDimension[0]) cursor = 'e-resize';
            else if (dimension[1] > frameDimension[1]) cursor = 'n-resize';
        ...
        self.fitToFrame = function (width, height) {
            //width and height of image
            if (typeof width == 'undefined' || typeof height == 'undefined') {
        ...
        self.zoomTo = function (newZoomLevel, x, y) {
            ...
            if (newZoomLevel < 0 || x < 0 || y < 0 || x >= frameDimension[0] || y >= frameDimension[1]) return false;

            var dimension = self.fitToFrame(orignalW, orignalH);
            for (var i = newZoomLevel; i > 0; i--) (dimension[0] *= zoomFactor), (dimension[1] *= zoomFactor);
        ...
        self.centerImage = function (width, height, x, y) {
            ...
            if (width <= frameDimension[0]) x = Math.round((frameDimension[0] - width) / 2);
            if (height <= frameDimension[1]) y = Math.round((frameDimension[1] - height) / 2);

            if (width > frameDimension[0]) {
                if (x > 0) x = 0;
                else if (x + width < frameDimension[0]) x = frameDimension[0] - width;
            }

            if (height > frameDimension[1]) {
                if (y > 0) y = 0;
                else if (y + height < frameDimension[1]) y = frameDimension[1] - height;
            }
        ...
        self.initImage = function () {
            ...
            if (frame[2] == true) self.frameElement.style.width = Math.round(dimension[0]) + 'px';
            if (frame[3] == true) self.frameElement.style.height = Math.round(dimension[1]) + 'px';
        ...
        if (image != null) {
            ...
            if (parent != null) parent.appendChild(self.frameElement);
        ...
        self.loadToolbar = function (self) {
            ...
            center.tabIndex = '1';
            center.src =
                'data:image/png;base64,'; //toolbarImages+'/center.png';
    ...
    function getObjectXY(object) {
        ...
        if (object.offsetParent) {
            ...
            do {
                ...
                if (object.style.borderLeftWidth != '') left += parseInt(object.style.borderLeftWidth);
                else object.style.borderLeftWidth = '0px';
                top += object.offsetTop;
                if (object.style.borderTopWidth != '') top += parseInt(object.style.borderTopWidth);
                else object.style.borderTopWidth = '0px';
    ...
    function mouseWheel() {
        ...
        self.wheelHandler = function (event) {
            ...
            if (delta) wheelCallback(event, this, delta); //callback function
        };
